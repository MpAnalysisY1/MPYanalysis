...
for line in lines:
    items=line.split("\t")
    if len(items)==5 and "total depth" in items[1]:
        gd_dict[items[0]]=items[3]
    elif len(items)==5 and "total depth" not in items[1]:
    ...
    if len(site_gts)==1:
        ...
        if "AMEL" in gt or "Yindel" in gt:
            ...
            if temp_gt1 and temp_gt2:
                outfile.write("%s\t%s\t%s\t\t%s\t%s:%s\t%s;%s\n"%(gt,temp_gt1,temp_gt2,d1+d2,d1,d2,gtstr1,gtstr2))
            elif temp_gt1:
            ...
            if len(alt)==1:
                ...
                if float(rate[0])>30:
                    if float(rate[1])<30 or float(alt[0][0])>float(temp_gt1):
                        temp_gt2=alt[0][0]
                        gtstr2=alt[0][2]
            ...
            outfile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s;%s\n"%(gt,temp_gt1,temp_gt2,",".join(rate),gd_dict[gt],";".join(depthlist),gtstr1,gtstr2))
    else:
